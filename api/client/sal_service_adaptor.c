...
API int service_plugin_destroy(service_plugin_h plugin)
{
	...
	service_adaptor->plugins = g_list_remove(service_adaptor->plugins, (gpointer)plugin);
...
API int service_plugin_start(service_plugin_h plugin, int service_mask)
{
	...
	RETV_IF(NULL == service_adaptor->uri, SERVICE_ADAPTOR_ERROR_UNKNOWN);
	RETV_IF(NULL == plugin->uri, SERVICE_ADAPTOR_ERROR_UNKNOWN);

	char *plugin_handle = NULL;
	int ret = ipc_service_plugin_start(service_adaptor->pid,
			service_adaptor->uri, plugin->uri, &plugin_handle);
