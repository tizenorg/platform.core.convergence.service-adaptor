...
{
	last_error_code = 0;
	snprintf(last_error_message, ERROR_MESSAGE_MAX_LENGH, "");
}

API void sal_ipc_client_set_last_error(int error, const char *message)
{
	sal_error("<thread-safe> set last error : [%d][%s]", error, message);
...
API int sal_ipc_client_get_simple_response(GVariant *response)
{
    ...
    GVariant *reply_info[reply_size] = {NULL, };
	...
	switch (ret) {
	case SAL_ERROR_NONE:
		sal_debug("case : SAL_ERROR_NONE");
		break;
	case SAL_ERROR_PLUGIN_FAILED:
		sal_debug("case : SAL_ERROR_PLUGIN_FAILED");
